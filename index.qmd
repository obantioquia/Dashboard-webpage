---
title: "Sistema de alertas tempranas de deforestación"
lang: es
format: 
  dashboard:
...
data_historico <- read.csv("Data/data_historico.csv")
...
<span class="title-style"> Reporte `r 1` </span>
...
leaflet() |> 
  ...
  addEsriFeatureLayer(
    ...
    weight = 1,
    group = "Veredas") |>
  addCircleMarkers(data = AlertasGFW_cluster, 
             lng = st_coordinates(AlertasGFW_cluster)[,1], 
             lat = st_coordinates(AlertasGFW_cluster)[,2], 
             color = "#f50060",  
             weight = 4,
             group = "Sistema GFW",
  ...
  addCircleMarkers(data = atdIDEAM, 
             lng = st_coordinates(atdIDEAM)[,1], 
             lat = st_coordinates(atdIDEAM)[,2], 
             color = "#ffd61e",  
             weight = 4,
             group = "Sistema IDEAM",
             clusterOptions = markerClusterOptions(iconCreateFunction=JS("function (cluster) {    
  ...
  hideGroup(c(paste("Imagen Planet", FechaImagen),
              "Municipios", "Veredas"
              #, "Superficie Bosque 2021"
              )) |>
  ...
  addCircleLegend(
    ...
    borderColors = c("#f50060", "#ffd61e", "#ff0000"),  # Colores de los bordes
...
list(
  ...
  color = "#A93154FF",
  value = format(dim(AlertasGFW_cluster)[1], big.mark = ".", decimal.mark = ",")
)
```
...
Periodo analizado: `r paste(unique(atdIDEAM$FechaInit), "-", unique(atdIDEAM$FechaFin))`
:::
...
::: card
Este reporte revela información de alertas tempranas de deforestación en el departamento de Antioquia, proveniente de dos sistemas, Global Forest Watch (GFW) y el Instituto de Hidrología, Meteorología, y Estudios Ambientales (IDEAM).

En esta primera parte del informe se muestran las alertas provenientes de ambos sistemas para un mismo periodo de tiempo, que corresponde a las últimas dos semanas en las cuales el IDEAM ha publicado las Alertas Tempranas de Deforestación (AT-D). En el periodo de tiempo analizado **`r paste(unique(atdIDEAM$FechaInit), "-", unique(atdIDEAM$FechaFin))`**, se encontró un total de **`r format(dim(AlertasGFW_cluster)[1], big.mark = ".", decimal.mark = ",")` alertas GFW** distribuidas en **`r dim(AlertasMunicipios)[1]` municipios**, y **`r format(dim(atdIDEAM)[1], big.mark=".", decimal.mark = ",")` alertas IDEAM** distribuidas en **`r dim(AlertasMunicipios_IDEAM)[1]` municipios**.
:::
...
```{r}
#| title: Histórico de alertas tempranas de deforestación
...
plot_ly(Data_Historico, x = ~FechaPublicacion) %>%
...
geovisor <- leaflet(shared_alertas, width = "100%", height = 400) |> 
  ...
  addCircleMarkers(color="#f50060", group = "Sistema GFW",
                   radius = 1,
                   weight = 2,
                   popup = ~paste0(y_coord,"°N; ",x_coord,"°W")) |>
  addLayersControl(overlayGroups = c("Municipios",
                                     "Veredas",
                                     "Sistema GFW",
                                     paste("Imagen Planet", FechaImagen)
